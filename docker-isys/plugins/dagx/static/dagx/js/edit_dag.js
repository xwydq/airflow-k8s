...
function readInfoData(id) {
    ...
    $("#" + id).find("[name]").each(function() {
        ...
        if (type == "text" || type == "number" ) {
            ...
            data[name] = $(this).is(':checked').toString();
            log('checkbox value ' + name + data[name]);
        } else if (type == "editor") {
            var editor = ace.edit($(this).attr("id"));
...
function checkDag() {
    ...
    var reCrond = new RegExp(/^(\*|([0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])|\*\/([0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])) (\*|([0-9]|1[0-9]|2[0-3])|\*\/([0-9]|1[0-9]|2[0-3])) (\*|([1-9]|1[0-9]|2[0-9]|3[0-1])|\*\/([1-9]|1[0-9]|2[0-9]|3[0-1])) (\*|([1-9]|1[0-2])|\*\/([1-9]|1[0-2])) (\*|([0-6])|\*\/([0-6]))$/);

    if (! reName.test(gDag.name)) {
        return 'dag 命名错误';
    }
    if (! reCrond.test(gDag.crond.trim())) {
        return 'dag 调度时间设置无效';
    }
