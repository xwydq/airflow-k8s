...
try:
    # airflow_db = os.environ['MYSQL_DB']
    airflow_db = configuration.get("core", "sql_alchemy_conn")
    airflow_db = airflow_db.split('/')[-1].split('?')[0]
except:
    airflow_db = 'airflow'
...
class DAGConverter(object):
    ...
    def get_deleted_dagpy(self, session=None):
        ...
        daglist_intab.append('dagx_converter')
    ...
    def get_dagconfs_render(self, session=None):
        ...
        for dag_conf in dag_confs:
            ...
            if conf:
                ...
                conf['emails'] = dag_conf.emails
                conf['email_on_skip_dag'] = False
                conf['skip_dag_on_prev_running'] = False
                # conf['skip_dag_not_latest'] = bool(dag_conf.skip_dag_not_latest)
                conf['skip_dag_not_latest'] = False
                conf['add_end_task'] = False
                conf['add_start_task'] = False
                conf['email_on_failure'] = bool(dag_conf.email_on_failure)
                conf['email_on_retry'] = bool(dag_conf.email_on_retry)
                conf['depends_on_past'] = bool(dag_conf.depends_on_past)
                conf['category'] = 'default'
                conf['retries'] = dag_conf.retries
                conf['retry_delay_minutes'] = dag_conf.retry_delay_minutes
                conf['concurrency'] = dag_conf.concurrency
                conf['max_active_runs'] = dag_conf.max_active_runs
    ...
    def delete_dag(self, session=None, dag_name=None):
        ## 删除 DAG文件/logs
        if dag_name:
            dag_py = os.path.join(dcmp_settings.DAG_CREATION_MANAGER_DEPLOYED_DAGS_FOLDER, dag_name + '.py')
            dag_log = os.path.join(base_log_folder, dag_name)
            if os.path.exists(dag_py):
                os.remove(dag_py)
            if os.path.exists(dag_log):
                shutil.rmtree(dag_log)
            # else:
        ...
        if dag_del:
            ...
            with dcmp_settings.CONN_AF.connect() as con:
                con.execute('delete from ' + airflow_db + '.xcom where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.task_instance where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.sla_miss where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.log where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.job where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.dag_run where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.log where dag_id = "%s"' % dag_del.name)
                con.execute('delete from ' + airflow_db + '.dag where dag_id = "%s"' % dag_del.name)
